...
public class BonusClaimsCommand {
                ...
                .then(Commands.argument("target", EntityArgument.player())
                    .then(Commands.literal("plus")
                        .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                            .executes(context -> modifyClaims(context, IntegerArgumentType.getInteger(context, "amount"))))
                    )
    ...
    private static int modifyClaims(CommandContext<CommandSourceStack> context, int amount) throws CommandSyntaxException {
        ...
        playerConfigAPI.tryToSet(PlayerConfigOptions.BONUS_CHUNK_CLAIMS, newAmount);

        String key = amount > 0 ? "message.opacbonusclaims.claims.added" : "message.opacbonusclaims.claims.removed";
